...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) echo "deb http://deb.debian.org/debian buster-backports main" >> /etc/apt/sources.list.d/backports.list && echo "deb http://deb.debian.org/debian stretch-backports main contrib" >> /etc/apt/sources.list.d/stretch-backports.list && apt update -y; break;;
        No ) break;;
    esac
done
